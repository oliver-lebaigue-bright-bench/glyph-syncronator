...
class CompanionApp(ctk.CTk):
    def __init__(self):
        ...
        self._update_loop()
        self.log("Ready. For system audio, please use BlackHole.")
    ...
    def _check_blackhole_status(self):
        ...
        if not has_blackhole:
            if messagebox.askyesno("BlackHole Missing", 
                                  "Glyphix requires the 'BlackHole' virtual driver to capture system audio on macOS.\n\n"
    ...
    def _setup_ui(self):
        ...
        self.logo_label = ctk.CTkLabel(self.header, text="GLYPHIX", font=ctk.CTkFont(family=FONT_LOGO, size=28, weight="bold"), text_color=COLOR_ACCENT)
        ...
        self.status_pill = ctk.CTkLabel(self.status_frame, text="DISCONNECTED", text_color=COLOR_MUTED, font=ctk.CTkFont(family=FONT_MAIN, size=11, weight="bold"))
        ...
        self.audio_combo = ctk.CTkOptionMenu(self.last_card, values=[], fg_color=COLOR_BORDER, button_color=COLOR_BORDER, button_hover_color=COLOR_ACCENT, dropdown_fg_color=COLOR_CARD, font=ctk.CTkFont(family=FONT_MAIN, size=12))
        ...
        ctk.CTkRadioButton(conn_switch_frame, text="UDP (Wi-Fi)", variable=self.conn_type, value="UDP", hover_color=COLOR_ACCENT, fg_color=COLOR_ACCENT, font=ctk.CTkFont(family=FONT_MAIN, size=12)).pack(side="left", padx=(0, 20))
        ctk.CTkRadioButton(conn_switch_frame, text="Bluetooth (Unsupported)", variable=self.conn_type, value="BT", hover_color=COLOR_ACCENT, fg_color=COLOR_ACCENT, font=ctk.CTkFont(family=FONT_MAIN, size=12), state="disabled").pack(side="left")
        addr_frame = ctk.CTkFrame(self.last_card, fg_color="transparent")
        addr_frame.pack(fill="x", padx=20, pady=(0, 20))
        self.addr_entry = ctk.CTkEntry(addr_frame, placeholder_text="IP Address", fg_color=COLOR_BG, border_color=COLOR_BORDER, height=36, font=ctk.CTkFont(family=FONT_MAIN, size=12))
        self.addr_entry.pack(side="left", fill="x", expand=True, padx=(0, 10))
        self.discover_btn = ctk.CTkButton(addr_frame, text="DISCOVER", width=90, height=36, fg_color=COLOR_WHITE, text_color=COLOR_BG, hover_color=COLOR_ACCENT, font=ctk.CTkFont(family=FONT_MAIN, size=12, weight="bold"), command=self._toggle_discovery)
        ...
        self.openrgb_switch = ctk.CTkSwitch(hw_grid, text="OpenRGB Sync", variable=self.use_openrgb, progress_color=COLOR_ACCENT, font=ctk.CTkFont(family=FONT_MAIN, size=12), command=self._toggle_openrgb)
        self.openrgb_switch.pack(side="left", padx=(0, 30))
        self.typing_switch = ctk.CTkSwitch(hw_grid, text="Typing Suppression", variable=self.typing_suppression, progress_color=COLOR_ACCENT, font=ctk.CTkFont(family=FONT_MAIN, size=12))
        ...
        ctk.CTkSlider(slider_frame, from_=0.1, to=3.0, variable=self.rgb_sensitivity, button_color=COLOR_WHITE, button_hover_color=COLOR_ACCENT, progress_color=COLOR_ACCENT).pack(fill="x", pady=(2, 12))
        ...
        ctk.CTkSlider(slider_frame, from_=0.5, to=0.99, variable=self.rgb_decay, button_color=COLOR_WHITE, button_hover_color=COLOR_ACCENT, progress_color=COLOR_ACCENT).pack(fill="x", pady=(2, 0))
        self.adv_btn = ctk.CTkButton(self.main_container, text="SHOW LOGS", fg_color="transparent", text_color=COLOR_MUTED, hover_color=COLOR_CARD, font=ctk.CTkFont(family=FONT_MAIN, size=10, weight="bold"), command=self._toggle_advanced)
        self.adv_btn.pack(pady=10)
        self.console = ctk.CTkTextbox(self.main_container, height=120, fg_color=COLOR_BG, border_color=COLOR_BORDER, border_width=1, font=ctk.CTkFont(family="Courier", size=10), text_color=COLOR_MUTED)
        self.console.configure(state="disabled")
        self.footer = ctk.CTkFrame(self, fg_color=COLOR_BG, height=80, corner_radius=0)
        self.footer.grid(row=2, column=0, sticky="ew")
        self.stream_btn = ctk.CTkButton(self.footer, text="START STREAMING", height=50, corner_radius=10, fg_color=COLOR_ACCENT, font=ctk.CTkFont(family=FONT_MAIN, size=14, weight="bold"), hover_color="#B0151A", command=self._toggle_streaming)
    ...
    def _toggle_advanced(self):
        if self.show_advanced.get():
            ...
            self.adv_btn.configure(text="SHOW LOGS")
            self.show_advanced.set(False)
        else:
            self.console.pack(fill="x", padx=20, pady=(0, 20))
            self.adv_btn.configure(text="HIDE LOGS")
    ...
    def _update_loop(self):
        ...
        if last_points:
            ...
            for i in range(num_dots): self.spectrum_points[i] = max(self.spectrum_points[i] * 0.8, interpolated[i])
        else:
            for i in range(num_dots): self.spectrum_points[i] *= 0.8
        self._draw_viz(); self.after(30, self._update_loop)
    ...
    def _resize_viz(self, event=None):
        ...
        if w <= 1: return
        target_spacing = 12; new_count = max(8, w // target_spacing)
        if new_count != len(self.viz_dots):
            self._init_viz_dots(new_count); self._viz_cache = [-1] * new_count
        dot_spacing = w / new_count; dot_size = max(4, dot_spacing - 4)
        for i in range(new_count):
            x = i * dot_spacing + dot_spacing/2
            for d in range(8):
                dy = h - (d * 10 + 15); dot = self.viz_dots[i][d]
    ...
    def _draw_viz(self):
        ...
        if len(self._viz_cache) != num_cols: self._viz_cache = [-1] * num_cols
        for i in range(num_cols):
            val = self.spectrum_points[i]; dots_to_draw = int(val * 8)
